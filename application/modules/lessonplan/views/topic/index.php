                            ...
                            <div class="x_content"> 
                               <?php echo form_open(site_url('lessonplan/topic/add'), array('name' => 'add', 'id' => 'add_topic', 'class'=>'form-horizontal form-label-left'), ''); ?>
                                ...
                                <div class="item form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <select  class="form-control col-md-12 col-xs-12 gsms-nice-select_"  name="subject_id"  id="add_subject_id" required="required"  onchange="get_lesson_by_subject(this.value, '', false);">
                                            <option value="">--<?php echo $this->lang->line('select'); ?>--</option>                                                                                         
                                        </select>
                            ...
                            <div class="x_content"> 
                               <?php echo form_open(site_url('lessonplan/topic/edit/'.$topic->id), array('name' => 'edit', 'id' => 'edit', 'class'=>'form-horizontal form-label-left'), ''); ?>
                                    ...
                                    <div class="item form-group">  
                                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="subject_id"><?php echo $this->lang->line('subject'); ?> <span class="required">*</span></label>
                                        <div class="col-md-6 col-sm-6 col-xs-12">
                                            <select  class="form-control col-md-12 col-xs-12 gsms-nice-select_"  name="subject_id"  id="edit_subject_id" required="required" onchange="get_topic_by_subject(this.value, '', true);">
                                                <option value="">--<?php echo $this->lang->line('select'); ?>--</option>                                                                                         
                                            </select>
                                            <div class="help-block"><?php echo form_error('subject_id'); ?></div>
                                        </div>
...
<script type="text/javascript">
 ...
 <script type="text/javascript">  
   ...
   <?php if(isset($topic)){?>
        get_lesson_by_subject('<?php echo $topic->subject_id; ?>', '<?php echo $topic->lesson_detail_id; ?>', true);
    <?php } ?>    
    function get_lesson_by_subject(subject_id, lesson_detail_id, is_edit){       
           
        $.ajax({       
            type   : "POST",
            url    : "<?php echo site_url('ajax/get_lesson_by_subject'); ?>",
            data   : { subject_id : subject_id , lesson_detail_id: lesson_detail_id},               
            async  : false,
            success: function(response){                                                   
               if(response)
               {                   
                  if(is_edit){
                       $('#edit_lesson_detail_id').html(response);
                   }else{
                       $('#add_lesson_detail_id').html(response);
                   }
               }
            }
        });          
