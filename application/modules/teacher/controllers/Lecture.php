...
class Lecture extends MY_Controller {
    ...
    * ********************************************************** */
    public function index($class_id = null) {
        ...
        $this->data['classes'] = $this->lecture->get_list('classes',array('status'=>1), '','', '', 'id', 'ASC');
        $this->data['class_list'] = $this->data['classes'];
         
        $this->data['class_id'] = $class_id;
        $this->data['filter_class_id'] = $class_id;
        
        $this->data['lectures'] = $this->lecture->get_lecture_list($class_id);
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {
                    ...
                    create_log('Has been uploaded an lecture : '.$data['title']);                     
                    success($this->lang->line('insert_success'));
                    redirect('teacher/lecture/index/'.$data['class_id']);
                    
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('teacher/lecture/add/'.$data['class_id']);
                }
            } else {
                error($this->lang->line('insert_failed'));
    ...
    * ********************************************************** */
    public function edit($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
                    ...
                    create_log('Has been updated a video lecture : '.$data['title']);                    
        ...
        $this->data['class_id'] = $this->data['lecture']->class_id;
        $this->data['filter_class_id'] = $this->data['lecture']->class_id;
        $this->data['lectures'] = $this->lecture->get_lecture_list($this->data['class_id']);
    ...
    private function _upload_lecture_ppt() {
        ...
        if ($lecture_ppt != "") {
            if ($lecture_ppt_type == 'application/powerpoint' ||
                ...
                $lecture_ppt_path = 'lecture-ppt-' . time() . '.' . $extension;
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        if ($this->lecture->delete('video_lectures', array('id' => $id))) {
            ...
            if (file_exists($destination . '/video-lecture/' . $lecture->lecture_ppt)) {
                @unlink($destination . '/video-lecture/' . $lecture->lecture_ppt);
            }
        ...
        redirect('teacher/lecture/index/' . $lecture->class_id);
    }
