...
/* * *****************Liveclass.php**********************************
 * @product name    : Global Multi Brunch School Management System Express
 * @type            : Class
...
class Liveclass extends MY_Controller {
    ...
    * ********************************************************** */
    public function index($class_id = null ) {
        ...
        $this->data['classes'] = $this->liveclass->get_list('classes', array('status'=>1), '','', '', 'id', 'ASC');
        $this->data['teachers'] = $this->liveclass->get_list('teachers',array('status'=>1), '','', '', 'id', 'ASC');
    ...
    * ********************************************************** */
    public function add() {
        ...
        $this->data['live_classes'] = $this->liveclass->get_single_live_class($class_id);
        $this->data['classes'] = $this->liveclass->get_list('classes',array('status'=>1), '','', '', 'id', 'ASC');
        $this->data['teachers'] = $this->liveclass->get_list('teachers', array('status'=>1), '', '', '', 'id', 'ASC');
        $this->data['class_list'] = $this->data['classes'];
    ...
    * ********************************************************** */
    public function edit($id = null) {
        ...
        $this->data['class_id'] = $this->data['live_class']->class_id;
        $this->data['filter_class_id'] = $this->data['live_class']->class_id;
        $this->data['live_classes'] = $this->liveclass->get_live_class_list($this->data['class_id']);
    ...
    * ********************************************************** */ 
    public function teacher_id() {
        ...
        if ($this->input->post('id') == '') {
            $schedule = $this->liveclass->duplicate_check($class_id, $section_id, $subject_id, $teacher_id, $class_date, $start_time);
            if ($schedule) {
            ...
            $schedule = $this->liveclass->duplicate_check($class_id, $section_id, $subject_id, $teacher_id, $class_date, $start_time, $this->input->post('id'));
            if ($schedule) {
                $this->form_validation->set_message('teacher_id', $this->lang->line('already_exist'));
                return FALSE;
            } else {
   ...
   public function _send_email_notification($data = null) {
            ...
            $email_setting = $this->liveclass->get_single('email_settings', array('status' => 1));
            $setting = $this->liveclass->get_single('settings', array('status' => 1));
           ...
           if(!empty($email_setting)){
            ...
            $students = $this->liveclass->get_student_list($data['class_id'], $data['section_id']);
            ...
            foreach ($students as $obj) {                
                ...
                if($obj->email != ''){
                    ...
                    $subject = $this->lang->line('live_class'). ' '. $this->lang->line('for') . ' ' ;
    ...
    * ********************************************************** */
    public function start($id = null) {
        ...
        if (strtotime($this->data['live_class']->class_date) == strtotime(date('Y-m-d')) && (strtotime(date('Y-m-d') .' '. $this->data['live_class']->start_time)) <= (strtotime(date('Y-m-d g:i A'))) && (strtotime(date('Y-m-d') .' '. $this->data['live_class']->end_time)) >= (strtotime(date('Y-m-d g:i A')))) {
            ...
            $this->layout->title($this->lang->line('live_class') . ' | ' . SMS);
            ...
            }else if($this->data['live_class']->class_type == 'google'){
                redirect($this->data['live_class']->meeting_url);
