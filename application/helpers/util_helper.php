...
if (!function_exists('get_invoice_items')) {
    ...
    function get_invoice_items($invoice_id, $invoice_type = null){
        
        $ci = & get_instance();
        if($invoice_type == 'sale'){
            
            $ci->db->select('IS.*, IH.title, C.name AS category, P.name as product');
            $ci->db->from('item_sales AS IS');        
            $ci->db->join('income_heads AS IH', 'IH.id = IS.income_head_id', 'left');
            $ci->db->join('item_categories AS C', 'C.id = IS.category_id', 'left');
            $ci->db->join('item_products AS P', 'P.id = IS.product_id', 'left');
            $ci->db->where('IS.invoice_id', $invoice_id);
            
        }else{
            $ci->db->select('ID.*, IH.title');
            $ci->db->from('invoice_detail AS ID');        
            $ci->db->join('income_heads AS IH', 'IH.id = ID.income_head_id', 'left');
...
if (!function_exists('get_routines_by_day')) {
    ...
    function get_routines_by_day($day, $class_id =null, $section_id = null, $academic_year_id = null) {
        ...
        if(logged_in_role_id() != SUPER_ADMIN && logged_in_role_id() != ADMIN){
...
if (!function_exists('get_lowet_height_mark')) {
    ...
    function get_lowet_height_mark($academic_year_id, $exam_id, $class_id, $section_id = null, $subject_id = null) {
        ...
        $ci->db->from('marks AS M');
        $ci->db->join('students AS ST', 'ST.id = M.student_id', 'left');
        $ci->db->where('M.academic_year_id', $academic_year_id);
        $ci->db->where('M.exam_id', $exam_id);
        $ci->db->where('M.class_id', $class_id);
        if($section_id){
        ...
        $ci->db->where('ST.status_type', 'regular');
        return  $ci->db->get()->row();
    }
...
if (!function_exists('get_lowet_height_result')) {
    ...
    function get_lowet_height_result($academic_year_id, $exam_id, $class_id, $section_id = null, $student_id = null) {
        ...
        $ci->db->join('students AS ST', 'ST.id = ER.student_id', 'left');
...
if (!function_exists('get_user_by_role')) {
    ...
    function get_user_by_role($role_id, $user_id) {
        ...
        if ($role_id == STUDENT) {
            ...
            $ci->db->select('T.*, U.email, D.title AS department, U.role_id, R.name AS role, SG.grade_name');
...
if (!function_exists('get_total_used_leave')) {
    ...
    function get_total_used_leave($role_id, $type_id, $user_id) {
        ...
        $ci->db->where('A.academic_year_id', $ci->academic_year_id);
...
if (!function_exists('get_video_types')) {
    ...
    function get_video_types() {
        ...
        return array(
            ...
            'power_point' => $ci->lang->line('power_point')
        );
    }
}
...
if (!function_exists('get_student_due_fees')) {

    function get_student_due_fees($academic_year_id, $class_id, $student_id) {
        
        $ci = & get_instance();
        $ci->db->select('I.month, I.net_amount, SUM(T.amount) AS paid_amount, IH.title AS head, C.name AS class_name, ST.name AS student,  AY.session_year');
        $ci->db->from('invoices AS I');   
        $ci->db->join('transactions AS T', 'T.invoice_id = I.id', 'left');
        $ci->db->join('students AS ST', 'ST.id = I.student_id', 'left');
        ...
        if($class_id != ''){
           $ci->db->where('I.class_id', $class_id);
        } 
        if($student_id != ''){
        ...
        if($academic_year_id){
            ...
            $ci->db->where('I.academic_year_id', $ci->academic_year_id);
        }        
        $ci->db->where('I.paid_status !=', 'paid');
        $ci->db->group_by('I.id', 'DESC'); 
...
if (!function_exists('get_teacher_rating')) {
    function get_teacher_rating($techer_id) {
        ...
        $ci->db->where('R.academic_year_id', $ci->academic_year_id);    
        return $ci->db->get()->row();     
    }
}
...
if (!function_exists('get_teacher_avg_rating')) {
    function get_teacher_avg_rating($techer_id) {
    ...
    function get_question_detail($online_exam_id, $question_id, $counter = null) {
        ...
        $str = '<div class="question-container">'; 
            $str .= '<div class="row question-title">';                
                ...
                $str .= '<div class="col-md-3 col-sm-3 col-xs-3 text-right">Mark: '.$question->mark.'</div>';
            $str .= '</div>';
...
if (!function_exists('get_question_type')) {
    function get_question_type() {
        ...
        $type['boolean'] = $ci->lang->line('true_flase');
